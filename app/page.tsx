...
export default function Home() {
  ...
  useEffect(() => {
    // Start animation sequence after 1.5 seconds
    const timer = setTimeout(() => {
      setShowAnimation(true)
    }, 700)

    // Show tiles after logo animation completes
  ...
  const services = [
    ...
    },
    {
      ...
      description: "Transporty ciężarowe",
      href: "/transport",
    ...
    <main className={`h-screen w-screen overflow-hidden ${isColorfulTheme ? "bg-background" : "bg-white"}`}>
      ...
      >
        <div className="text-center">
          ...
          <p className="text-2xl text-primary font-semibold">Centrum Motoryzacyjne</p>
        </div>
      </div>

      <div
        className={`fixed top-6 right-6 z-40 transition-all duration-500 ${showTiles ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      >
        <button
          onClick={() => setIsColorfulTheme(!isColorfulTheme)}
          className="flex items-center gap-3 bg-white/90 backdrop-blur-sm px-4 py-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border border-gray-200"
          aria-label="Przełącz kolorystykę"
        >
          <span className="text-sm font-medium text-gray-700">{isColorfulTheme ? "Kolorowa" : "Firmowa"}</span>
          <div className="relative w-12 h-6 bg-gray-300 rounded-full transition-colors duration-300">
            <div
              className={`absolute top-1 left-1 w-4 h-4 bg-primary rounded-full transition-transform duration-300 ${isColorfulTheme ? "translate-x-0" : "translate-x-6"}`}
            />
          </div>
        </button>
      </div>

      {/* Services Grid - Full Screen Tiles */}
      <div
        className={`absolute inset-0 z-10 transition-all duration-500 ${showTiles ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      >
        <div className={`h-screen w-screen grid grid-cols-2 ${isColorfulTheme ? "gap-0" : "gap-6 p-8"}`}>
          {services.map((service, index) => {
            ...
            const bgImages = isColorfulTheme ? colorfulBgImages : []
            ...
            return (
              ...
              >
                {/* Overlay - only for colorful theme */}
                {isColorfulTheme && (
      ...
      {/* Footer Social & Contact - Always visible */}
      <footer
        className={`fixed bottom-0 left-0 right-0 z-30 py-6 px-4 transition-all duration-500 ${showTiles ? "opacity-100" : "opacity-0"} ${
          isColorfulTheme ? "bg-gradient-to-t from-black/20 to-transparent" : "bg-transparent"
        }`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-center gap-6">
          <a
            href="https://facebook.com/lxt"
            target="_blank"
